...
Every view does ``import shared`` first; this module makes the repo root
importable (so ``lotterylab`` resolves without an install), and centralises the
cached data access plus the small formatting helpers the pages share.
"""
...
# Repo root = parent of app/. Inserted so `import lotterylab` works whether the
# app is launched via `streamlit run`, AppTest, or plain python.
...
from lotterylab.backtest import BacktestResult, backtest  # noqa: E402
from lotterylab.strategy import get_strategy  # noqa: E402
